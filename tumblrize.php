...
{
	echo "<div class=\"error\"><p>Tumblrize is not configured. Please update your <a href=\"".get_bloginfo('wpurl')."/wp-admin/options-general.php?page=tumblrize/tumblrize.php\">Tumblr email and password</a>.</p></div>";
}
...
<input type="hidden" name="page_options" value="tumblr_email,tumblr_password,tumblr_posterous,notify_me,add_permalink,shut_tumblrize,tumblrize_tags" />
...
// SEND Data
    ...
    if(get_option('tumblr_email') && get_option('tumblr_password') && $post_body){
		...
		// Notify Services if Post is new
		if( ( $_POST['post_status'] == 'publish' ) && ( $_POST['original_post_status'] != 'publish' ) ) {
			// What's notified?
			if(get_option('shut_tumblrize' ) == ""){ $tb_services = "Tumblr"; }
			if(get_option('tumblr_posterous') == "posterous"){ $tb_services .= " and Posterous"; }
			if(get_option('shut_tumblrize' ) && get_option('tumblr_posterous') == "posterous"){ $tb_services = "Posterous"; }
			// Notify TUMBLR if checked
			if(get_option('shut_tumblrize') == ""){			
			...
			// Notify USER if checked
   			if ( get_option('notify_me') == "notify_on" ) { $notify_email = get_option('tumblr_email'); tumblrize_notify($notify_email, $post_title, $post_body, $tb_services); }
		}
