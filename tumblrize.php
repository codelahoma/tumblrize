...
if (!defined('TUMBLRIZE_PLUGIN_VERISON')) {
    define('TUMBLRIZE_PLUGIN_VERSION', '1.3.2');
...
class TumblrizePlugin {
    ...
    function tumblrize($post_ID)  {
        ...
        if (!$this->isTumblrizeablePost()) { return $post_ID; }
        ...
        $post_body  = str_replace('\"', "", html_entity_decode($post->post_content));
        ...
        if ($this->tusername && $this->tpassword && $post_body) {
            ...
            switch ($post_type) {
                case 'photo':
                    ...
                    // TODO:
                    // I think we only really want "click-through-url" when
                    // we use direct file upload. Right?
    ...
    function delete_post ($post_ID) {
        $r = $this->tumblrize_to_tumblr('delete',
    ...
    function tumblrize_to_tumblr ($do, $data) {
        ...
        if ($this->isTumblrizeablePost() && $data) {
    ...
    function show_tr_warning () {
        print '<div class="error"><p>Tumblrize is not configured. Please provide your Tumblr email and password in the <a href="'.get_bloginfo('wpurl').'/wp-admin/options-general.php?page=tumblrize/tumblrize.php">Tumblrize Options</a> screen.</p></div>';
    }
...
if (get_option('tumblrize_tumblr_email') == "") {
    add_action('admin_notices', array($tumblrize, 'show_tr_warning'));
}
...
<fieldset>
    ...
    <div>
        ...
        <label for="tumblrize_do-tumblrize">No</label>
    </div>
...
function tumblrize_activate () {
    if (TUMBLRIZE_PLUGIN_VERSION != '1.3' || TUMBLRIZE_PLUGIN_VERSION != '1.3.1') {
...
<div class="wrap">
    ...
    <form method="post" action="options.php">
        ...
        <input type="hidden" name="page_options" value="tumblrize_tumblr_email,tumblrize_tumblr_password,tumblrize_tumblr_posterous,tumblrize_notify_me,tumblrize_add_permalink,tumblrize_shutoff,tumblrize_tags,tumblrize_add_post_tags,tumblrize_purge_database,tumblrize_tumblr_group" />
        <p class="submit">
