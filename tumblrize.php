...
class TumblrizePlugin {
    ...
    function tumblrize ($post_ID) {
        ...
        if (!$this->tusername || !$this->tpassword || !$post_body) {
            ...
            switch ($post_type) {
                ...
                case 'link':
                    if (!isset($link_url) || empty($link_url)) {
                        return $post_ID; // will forego posting a link; silently fails
                    }
                ...
                case 'audio':
                    if (!isset($audio_url) || empty($audio_url)) {
                        return $post_ID; // will forego posting audio; silently fails
                    }
                    $request_data['externally-hosted-url'] = $audio_url;
                    $request_data['caption'] = $post_body;
                case 'video':
                    if (!isset($post_video) || empty($post_video)) {
                        return $post_ID; // will forego posting a video; silently fails
                    }
            ...
            } else if ($r && $r['status'] === 500) { // Tumblr barfed?
                $this->add_admin_message('error',
                    'Tumblr returned an HTTP Status Code of 500. Tumblr might be having problems. Try crossposting again.');
            } else {
     ...
     *
     * @see function tumblrize_to_tumblr()
     */
